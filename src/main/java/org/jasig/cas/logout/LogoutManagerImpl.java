...
public final class LogoutManagerImpl implements LogoutManager {
    ...
    public List<LogoutRequest> performLogout(final TicketGrantingTicket ticket) {
        ...
        synchronized (ticket) {
            ...
            LOGGER.error("执行单点登出操作performLogout，所有的services为："+JSON.toJSONString(services));
        ...
        if (!disableSingleSignOut) {
        	LOGGER.error("disableSingleSignOut为false，开始执行单点登出操作");
                    ...
                    if (!singleLogoutService.isLoggedOutAlready()) {
                        ...
                        if (registeredService == null || registeredService.getLogoutType() == null
                            ...
                            if (performBackChannelLogout(logoutRequest)) {
                                ...
                                LOGGER.error("Logout message not sent to [{}]; Continuing processing...",
        	...
        	LOGGER.error("disableSingleSignOut为true，不执行单点登出操作");
        }
    ...
    private boolean performBackChannelLogout(final LogoutRequest request) {
        ...
        LOGGER.error("发送登出请求Sending logout request for: [{}]", request.getService().getId());
