...
public final class SimpleHttpClient implements HttpClient, Serializable, DisposableBean {
    ...
    private static final class MessageSender implements Callable<Boolean> {
        ...
        public Boolean call() throws Exception {
            ...
            try {
                LOGGER.error("尝试访问Attempting to access {}", url);
                ...
                LOGGER.error("已发送Finished sending message to {}", url);
                return true;
            } catch (final SocketTimeoutException e) {
                LOGGER.error("Socket Timeout Detected while attempting to send message to [{}]", url);
                return false;
            } catch (final Exception e) {
                LOGGER.error("Error Sending message to url endpoint [{}]. Error is [{}]", url, e.getMessage());
                return false;
            } finally {
